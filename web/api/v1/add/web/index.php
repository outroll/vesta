...
if (!empty($_POST['ok'])) {
    ...
    if (empty($_SESSION['error_msg'])) {
        exec (devit_CMD."v-add-web-domain ".$user." ".$v_domain." ".$v_ip." no ".$aliases." ".$proxy_ext, $output, $return_var);
    ...
    if (($_POST['v_dns'] == 'on') && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-add-dns-domain ".$user." ".$v_domain." ".$v_public_ip." '' '' '' '' '' '' '' '' no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if (($_POST['v_dns'] == 'on') && (empty($_SESSION['error_msg']))) {
        foreach ($aliases_arr as $alias) {
            if ($alias != "www.".$_POST['v_domain']) {
                ...
                exec (devit_CMD."v-add-dns-on-web-alias ".$user." ".$alias." ".$v_ip." no", $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
            }
        }
    ...
    if (($_POST['v_mail'] == 'on') && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-add-mail-domain ".$user." ".$v_domain, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((!empty($_SESSION['PROXY_SYSTEM'])) && ($_POST['v_proxy'] == 'off')  && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-delete-web-domain-proxy ".$user." ".$v_domain." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    // Add Lets Encrypt support
     if ((!empty($_POST['v_letsencrypt'])) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-schedule-letsencrypt-domain ".$user." ".$v_domain, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
     } else {
    ...
    if ((!empty($_POST['v_stats'])) && ($_POST['v_stats'] != 'none' ) && (empty($_SESSION['error_msg']))) {
        $v_stats = escapeshellarg($_POST['v_stats']);
        exec (devit_CMD."v-add-web-domain-stats ".$user." ".$v_domain." ".$v_stats, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((!empty($_POST['v_stats_user'])) && (empty($_SESSION['error_msg']))) {
        ...
        fclose($fp);
        exec (devit_CMD."v-add-web-domain-stats-user ".$user." ".$v_domain." ".$v_stats_user." ".$v_stats_password, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        ...
        foreach ($_POST['v_ftp_user'] as $i => $v_ftp_user_data) {
            if ($v_ftp_user_data['is_new'] == 1) {
                ...
                if ($domain_added) {
                    ...
                    fclose($fp);
                    exec (devit_CMD."v-add-web-domain-ftp ".$user." ".$v_domain." ".$v_ftp_user." ".$v_ftp_password . " " . $v_ftp_path, $output, $return_var);
                    check_return_code($return_var,$output);
                    unset($output);
