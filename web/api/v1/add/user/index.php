...
if (!empty($_POST['ok'])) {
    ...
    if (empty($_SESSION['error_msg'])) {
        ...
        fclose($fp);
        exec (devit_CMD."v-add-user ".$v_username." ".$v_password." ".$v_email." ".$v_package." ".$v_fname." ".$v_lname, $output, $return_var);
    ...
    // Set language
    if (empty($_SESSION['error_msg'])) {
        exec (devit_CMD."v-change-user-language ".$v_username." ".$v_language, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((empty($_SESSION['error_msg'])) && (!empty($v_notify))) {
        $to = $_POST['v_notify'];
        $subject = _translate($_POST['v_language'],"Welcome to DevIT Control Panel");
