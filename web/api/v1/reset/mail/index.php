...
exec (devit_CMD."v-list-sys-ips json", $output, $return_var);
...
if ((!empty($_POST['email'])) && (!empty($_POST['password'])) && (!empty($_POST['new']))) {
    ...
    // Get domain owner
    exec (devit_CMD."v-search-domain-owner ".$v_domain." mail", $output, $return_var);
    ...
    if (!empty($v_user)) {
        exec (devit_CMD."v-get-mail-account-value ".$v_user." ".$v_domain." ".$v_account." md5", $output, $return_var);
        if ($return_var == 0) {
            $v_hash = $output[0];
        }
    }
    ...
    if (!empty($v_hash)) {
        ...
        if ( hash_equals($v_hash, $n_hash ) ) {
            ...
            fwrite($fp, $_POST['new']."\n");
            fclose($fp);
            exec (devit_CMD."v-change-mail-account-password ".$v_user." ".$v_domain." ".$v_account." ".$v_new_password, $output, $return_var);
            if ($return_var == 0) {
                echo "==ok==";
                exit;
            }
