...
exec (devit_CMD."v-list-web-domain ".$user." ".$v_domain." json", $output, $return_var);
...
if (!empty($v_ssl)) {
    exec (devit_CMD."v-list-web-domain-ssl ".$user." ".escapeshellarg($v_domain)." json", $output, $return_var);
...
if (!empty($_POST['save'])) {
    ...
    if (($v_ip != $_POST['v_ip']) && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-change-web-domain-ip ".$v_username." ".$v_domain." ".$v_ip." no", $output, $return_var);
    ...
    // Chane dns domain IP
    if (($v_ip != $_POST['v_ip']) && (empty($_SESSION['error_msg'])))  {
        exec (devit_CMD."v-list-dns-domain ".$v_username." ".$v_domain." json", $output, $return_var);
        unset($output);
        if ($return_var == 0 ) {
            $v_ip = escapeshellarg($_POST['v_ip']);
            exec (devit_CMD."v-change-dns-domain-ip ".$v_username." ".$v_domain." ".$v_ip." no", $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
            $restart_dns = 'yes';
        }
    ...
    if (($v_ip != $_POST['v_ip']) && (empty($_SESSION['error_msg']))) {
        foreach($valiases as $v_alias ){
            ...
            exec (devit_CMD."v-list-dns-domain ".$v_username." ".$v_alias." json", $output, $return_var);
            unset($output);
            if ($return_var == 0 ) {
                $v_ip = escapeshellarg($_POST['v_ip']);
                exec (devit_CMD."v-change-dns-domain-ip ".$v_username." ".$v_alias." ".$v_ip, $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
                $restart_dns = 'yes';
            }
    ...
    if (($v_template != $_POST['v_template']) && ( $_SESSION['user'] == 'admin') && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-change-web-domain-tpl ".$v_username." ".$v_domain." ".$v_template." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $restart_web = 'yes';
    }
    ...
    if (empty($_SESSION['error_msg'])) {
        ...
        foreach ($result as $alias) {
            if ((empty($_SESSION['error_msg'])) && (!empty($alias))) {
                ...
                exec (devit_CMD."v-delete-web-domain-alias ".$v_username." ".$v_domain." ".$alias." no", $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);

                if (empty($_SESSION['error_msg'])) {
                    exec (devit_CMD."v-list-dns-domain ".$v_username." ".$v_domain, $output, $return_var);
                    unset($output);
                    if ($return_var == 0) {
                        exec (devit_CMD."v-delete-dns-on-web-alias ".$v_username." ".$v_domain." ".$alias." no", $output, $return_var);
                        check_return_code($return_var,$output);
                        unset($output);
                        $restart_dns = 'yes';
                    }
        ...
        foreach ($result as $alias) {
            if ((empty($_SESSION['error_msg'])) && (!empty($alias))) {
                ...
                $v_template = escapeshellarg($_POST['v_template']);
                $alias = escapeshellarg($alias);
                exec (devit_CMD."v-add-web-domain-alias ".$v_username." ".$v_domain." ".$alias." no", $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
                if (empty($_SESSION['error_msg'])) {
                    exec (devit_CMD."v-list-dns-domain ".$v_username." ".$v_domain, $output, $return_var);
                    unset($output);
                    if ($return_var == 0) {
                        exec (devit_CMD."v-add-dns-on-web-alias ".$v_username." ".$alias." ".$v_ip." no", $output, $return_var);
                        check_return_code($return_var,$output);
                    unset($output);
                        $restart_dns = 'yes';
                    }
    ...
    if ((!empty($_SESSION['WEB_BACKEND'])) && ( $v_backend_template != $_POST['v_backend_template']) && ( $_SESSION['user'] == 'admin') && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-change-web-domain-backend-tpl ".$v_username." ".$v_domain." ".escapeshellarg($v_backend_template), $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((!empty($_SESSION['PROXY_SYSTEM'])) && (!empty($v_proxy)) && (empty($_POST['v_proxy'])) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-delete-web-domain-proxy ".$v_username." ".$v_domain." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        unset($v_proxy);
        $restart_proxy = 'yes';
    ...
    if ((!empty($_SESSION['PROXY_SYSTEM'])) && (!empty($v_proxy)) && (!empty($_POST['v_proxy'])) && (empty($_SESSION['error_msg'])) && ($_SESSION['user'] == 'admin')) {
        ...
        if (( $v_proxy_template != $_POST['v_proxy_template']) || ($v_proxy_ext != $ext)) {
            ...
            exec (devit_CMD."v-change-web-domain-proxy-tpl ".$v_username." ".$v_domain." ".escapeshellarg($v_proxy_template)." ".escapeshellarg($ext)." no", $output, $return_var);
            check_return_code($return_var,$output);
    ...
    if ((!empty($_SESSION['PROXY_SYSTEM'])) && (empty($v_proxy)) && (!empty($_POST['v_proxy'])) && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-add-web-domain-proxy ".$v_username." ".$v_domain." ".escapeshellarg($v_proxy_template)." ".escapeshellarg($ext)." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $restart_proxy = 'yes';
    }
    ...
    if (( $v_ssl == 'yes') && (!empty($_POST['v_ssl'])) && (empty($_SESSION['error_msg']))) {
        if ( $v_ssl_home != $_POST['v_ssl_home'] ) {
            ...
            exec (devit_CMD."v-change-web-domain-sslhome ".$user." ".$v_domain." ".$v_ssl_home." no", $output, $return_var);
            check_return_code($return_var,$output);
    ...
    if (( $v_letsencrypt == 'no' ) && (empty($_POST['v_letsencrypt'])) && ( $v_ssl == 'yes' ) && (!empty($_POST['v_ssl'])) && (empty($_SESSION['error_msg']))) {
        if (( $v_ssl_crt != str_replace("\r\n", "\n",  $_POST['v_ssl_crt'])) || ( $v_ssl_key != str_replace("\r\n", "\n",  $_POST['v_ssl_key'])) || ( $v_ssl_ca != str_replace("\r\n", "\n",  $_POST['v_ssl_ca']))) {
            ...
            exec (devit_CMD."v-change-web-domain-sslcert ".$user." ".$v_domain." ".$tmpdir." no", $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
            $restart_web = 'yes';
            $restart_proxy = 'yes';

            exec (devit_CMD."v-list-web-domain-ssl ".$user." ".$v_domain." json", $output, $return_var);
            $ssl_str = json_decode(implode('', $output), true);
            unset($output);
            $v_ssl_crt = $ssl_str[$v_domain]['CRT'];
            $v_ssl_key = $ssl_str[$v_domain]['KEY'];
    ...
    if (( $v_letsencrypt == 'yes' ) && (empty($_POST['v_letsencrypt'])) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-delete-letsencrypt-domain ".$user." ".$v_domain." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_ssl_crt = '';
        $v_ssl_key = '';
    ...
    if (( $v_ssl == 'yes' ) && (empty($_POST['v_ssl'])) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-delete-web-domain-ssl ".$v_username." ".$v_domain." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_ssl_crt = '';
        $v_ssl_key = '';
    ...
    if ((!empty($_POST['v_ssl'])) && ( $v_letsencrypt == 'no' ) && (!empty($_POST['v_letsencrypt'])) && empty($_SESSION['error_msg'])) {
        ...
        exec (devit_CMD."v-add-letsencrypt-domain ".$user." ".$v_domain." ".escapeshellarg($l_aliases)." no", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_letsencrypt = 'yes';
        $v_ssl = 'yes';
     ...
     if (( $v_ssl == 'no' ) && (!empty($_POST['v_ssl']))  && (empty($v_letsencrypt_deleted)) && (empty($_SESSION['error_msg']))) {
        ...
        if (!empty($errors[0])) {
            ...
            exec (devit_CMD."v-add-web-domain-ssl ".$user." ".$v_domain." ".$tmpdir." ".$v_ssl_home." no", $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
            $v_ssl = 'yes';
            $restart_web = 'yes';
    ...
    if ((!empty($v_stats)) && ($_POST['v_stats'] == 'none') && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-delete-web-domain-stats ".$v_username." ".$v_domain, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_stats = '';
    }
    ...
    if ((!empty($v_stats)) && ($_POST['v_stats'] != $v_stats) && (empty($_SESSION['error_msg']))) {
        ...
        exec (devit_CMD."v-change-web-domain-stats ".$v_username." ".$v_domain." ".$v_stats, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((empty($v_stats)) && ($_POST['v_stats'] != 'none') && (empty($_SESSION['error_msg']))) {
        $v_stats = escapeshellarg($_POST['v_stats']);
        exec (devit_CMD."v-add-web-domain-stats ".$v_username." ".$v_domain." ".$v_stats, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
    ...
    if ((!empty($v_stats_user)) && (empty($_POST['v_stats_auth'])) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-delete-web-domain-stats-user ".$v_username." ".$v_domain, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        $v_stats_user = '';
        $v_stats_password = '';
    ...
    if ((empty($v_stats_user)) && (!empty($_POST['v_stats_auth'])) && (empty($_SESSION['error_msg']))) {
        ...
        if (!empty($errors[0])) {
            ...
            fclose($fp);
            exec (devit_CMD."v-add-web-domain-stats-user ".$v_username." ".$v_domain." ".$v_stats_user." ".$v_stats_password, $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
        ...
        foreach ($_POST['v_ftp_user'] as $i => $v_ftp_user_data) {
            ...
            if ($v_ftp_user_data['is_new'] == 1 && !empty($_POST['v_ftp'])) {
                ...
                if (empty($_SESSION['error_msg'])) {
                    ...
                    fclose($fp);
                    exec (devit_CMD."v-add-web-domain-ftp ".$v_username." ".$v_domain." ".$v_ftp_user." ".$v_ftp_password . " " . $v_ftp_path, $output, $return_var);
                    check_return_code($return_var,$output);
            ...
            if ($v_ftp_user_data['delete'] == 1) {
                ...
                exec (devit_CMD."v-delete-web-domain-ftp ".$v_username." ".$v_domain." ".$v_ftp_username, $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
            ...
            if (!empty($_POST['v_ftp'])) {
                ...
                $v_ftp_username = escapeshellarg($v_ftp_username);
                    ...
                    if(escapeshellarg(trim($v_ftp_user_data['v_ftp_path_prev'])) != $v_ftp_path) {
                        exec (devit_CMD."v-change-web-domain-ftp-path ".$v_username." ".$v_domain." ".$v_ftp_username." ".$v_ftp_path, $output, $return_var);
                    }
                ...
                if (!empty($v_ftp_user_data['v_ftp_password'])) {
                    ...
                    fwrite($fp, $v_ftp_user_data['v_ftp_password']."\n");
                    fclose($fp);
                    exec (devit_CMD."v-change-web-domain-ftp-password ".$v_username." ".$v_domain." ".$v_ftp_username." ".$v_ftp_password, $output, $return_var);
                    unlink($v_ftp_password);
    ...
    if ((!empty($_SESSION['PROXY_SYSTEM'])) && !empty($restart_proxy) && (empty($_SESSION['error_msg']))) {
        exec (devit_CMD."v-restart-proxy", $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
