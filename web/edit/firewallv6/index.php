...
// Check ip argument
if (empty($_GET['rule'])) {
    header("Location: /list/firewallv6/");
    exit;
}

// List rule
$v_rule = escapeshellarg($_GET['rule']);
exec (VESTA_CMD."v-list-firewall-ipv6-rule ".$v_rule." 'json'", $output, $return_var);
...
if (!empty($_POST['save'])) {
    ...
    // Change Status
    exec (VESTA_CMD."v-change-firewall-ipv6-rule ".$v_rule." ".$v_action." ".$v_ip."  ".$v_port." ".$v_protocol." ".$v_comment, $output, $return_var);
    check_return_code($return_var,$output);
    unset($output);
...
// Display body
include($_SERVER['DOCUMENT_ROOT'].'/templates/admin/edit_firewall_ipv6.html');
