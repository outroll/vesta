...
software="nginx httpd mod_ssl mod_ruid2 mod_fcgid mod_extract_forwarded
    ...
    which devit devit-nginx devit-php vim-common expect devit-ioncube
    devit-softaculous"
...
if [ "$exim" = 'yes' ]; then
    ...
    if [ "$clamd" = 'yes'  ]; then
        echo "ANTIVIRUS_SYSTEM='clamd.scan'" >> $devit/conf/devit.conf
    fi
    if [ "$spamd" = 'yes' ]; then
...
if [ "$clamd" = 'yes' ]; then
    ...
    gpasswd -a clam mail
    cp -r /etc/clamd.d $vst_backups/clamd >/dev/null 2>&1
    cp -f $devitcp/clamav/clamd.conf /etc/clamd.conf
