...
proftpd-basic bind9 exim4 exim4-daemon-heavy clamav-daemon
spamassassin dovecot-imapd dovecot-pop3d roundcube-core
roundcube-mysql roundcube-plugins mysql-common 
postgresql postgresql-contrib phppgadmin mc
flex whois rssh git idn zip sudo bc ftp lsof ntpdate rrdtool quota
e2fslibs bsdutils e2fsprogs curl imagemagick fail2ban dnsutils
bsdmainutils cron vesta vesta-nginx vesta-php expect s3cmd"
...
if [ "$apache" = 'yes'  ]; then
    ...
    cp -f /usr/local/vesta/install/ubuntu/$release/logrotate/apache2 /etc/logrotate.d/apache2
    a2enmod rewrite
    a2enmod suexec
    a2enmod ssl
    a2enmod actions
...
if [ "$postgresql" = 'yes' ]; then
    ...
    if [ "$apache" = 'yes' ]; then
        cp -f /usr/local/vesta/install/ubuntu/$release/pga/phppgadmin.conf /etc/apache2/conf.d/phppgadmin.conf
    fi
    cp -f /usr/local/vesta/install/ubuntu/$release/pga/config.inc.php /etc/phppgadmin/config.inc.php
fi
...
if [ "$exim" = 'yes' ]; then
    ...
    cp -f /usr/local/vesta/install/ubuntu/$release/exim/dnsbl.conf /etc/exim4/dnsbl.conf
    cp -f /usr/local/vesta/install/ubuntu/$release/exim/spam-blocks.conf /etc/exim4/spam-blocks.conf
...
if [ "$dovecot" = 'yes' ]; then
    ...
    cp -f /usr/local/vesta/install/ubuntu/$release/logrotate/dovecot /etc/logrotate.d/dovecot
...
if [ "$clamd" = 'yes' ]; then
    ...
    cp -f /usr/local/vesta/install/ubuntu/$release/clamav/clamd.conf /etc/clamav/clamd.conf
    /usr/bin/freshclam
