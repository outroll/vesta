...
if [ "$apache" = 'yes'  ]; then
    cp $vestacpinstalldir/apache2/apache2.conf /etc/apache2/apache2.conf
    cp $vestacpinstalldir/apache2/status.conf /etc/apache2/mods-enabled/status.conf
    cp $vestacpinstalldir/logrotate/apache2 /etc/logrotate.d/apache2
    a2enmod rewrite
    a2enmod suexec
    a2enmod ssl
    a2enmod actions
...
if [ "$phpfpm" = 'yes' ]; then
    ...
    cp $vestacpinstalldir/php-fpm/www.conf $pool/www.conf
...
if [ "$vsftpd" = 'yes' ]; then
    cp $vestacpinstalldir/vsftpd/vsftpd.conf /etc/vsftpd.conf
...
if [ "$mysql" = 'yes' ]; then
    ...
    cp $vestacpinstalldir/mysql/$mycnf /etc/mysql/my.cnf
    if [ "$release" != '16.04' ]; then
        mysql_install_db
    fi
    ...
    if [ "$apache" = 'yes' ]; then
        cp $vestacpinstalldir/pma/apache.conf /etc/phpmyadmin/apache.conf
        ln -s /etc/phpmyadmin/apache.conf /etc/apache2/conf.d/phpmyadmin.conf
    fi
    cp $vestacpinstalldir/pma/config.inc.php /etc/phpmyadmin/config.inc.php
...
if [ "$postgresql" = 'yes' ]; then
    cp $vestacpinstalldir/postgresql/pg_hba.conf /etc/postgresql/*/main/pg_hba.conf
    service postgresql restart
    sudo -u postgres psql -c "ALTER USER postgres WITH PASSWORD '$vpass'" 2>/dev/null

    # Configuring phpPgAdmin
    if [ "$apache" = 'yes' ]; then
        cp $vestacpinstalldir/pga/phppgadmin.conf /etc/apache2/conf.d/phppgadmin.conf
    fi
...
if [ "$named" = 'yes' ]; then
    cp $vestacpinstalldir/bind/named.conf /etc/bind/named.conf
...
if [ "$exim" = 'yes' ]; then
    ...
    cp $vestacpinstalldir/exim/exim4.conf.template /etc/exim4/exim4.conf.template
    cp $vestacpinstalldir/exim/dnsbl.conf /etc/exim4/dnsbl.conf
    cp $vestacpinstalldir/exim/spam-blocks.conf /etc/exim4/spam-blocks.conf
...
if [ "$clamd" = 'yes' ]; then
    ...
    cp $vestacpinstalldir/clamav/clamd.conf /etc/clamav/clamd.conf
    /usr/bin/freshclam
...
if [ "$exim" = 'yes' ] && [ "$mysql" = 'yes' ]; then
    if [ "$apache" = 'yes' ]; then
        cp $vestacpinstalldir/roundcube/apache.conf /etc/roundcube/apache.conf
        ln -s /etc/roundcube/apache.conf /etc/apache2/conf.d/roundcube.conf
    fi
    cp $vestacpinstalldir/roundcube/main.inc.php /etc/roundcube/main.inc.php
    cp $vestacpinstalldir/roundcube/db.inc.php /etc/roundcube/db.inc.php
    chmod 640 /etc/roundcube/debian-db-roundcube.php
    chown root:www-data /etc/roundcube/debian-db-roundcube.php
    cp $vestacpinstalldir/roundcube/vesta.php \
        /usr/share/roundcube/plugins/password/drivers/vesta.php
    cp $vestacpinstalldir/roundcube/config.inc.php \
...
if [ "$fail2ban" = 'yes' ]; then
    cd /etc
    cp -rf $vestacpinstalldir/fail2ban .
    if [ "$dovecot" = 'no' ]; then
