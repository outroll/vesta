...
rebuild_user_conf() {
    ...
    # Update disk pipe
    sed -i "/ $user$/d" $devit/data/queue/disk.pipe
...
rebuild_web_domain_conf() {
    ...
    if [ ! -z "$STATS" ]; then
        ...
        check_webstats=$(grep "$webstats" $devit/data/queue/webstats.pipe)
        if [ -z "$check_webstats" ]; then
            echo "$webstats" >> $devit/data/queue/webstats.pipe
        fi
...
rebuild_pgsql_database() {
    ...
    host_str=$(grep "HOST='$HOST'" $devit/conf/pgsql.conf)
    eval $host_str
...
import_mysql_database() {
    ...
    host_str=$(grep "HOST='$HOST'" $devit/conf/mysql.conf)
    eval $host_str
